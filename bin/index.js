...
async function install(pkgManager) {
...
async function chooseMod() {
    const response = await prompts({
        ...
        choices: [
            {
                title: 'Eslint+Prettier+Husky', value: 'eslint', description: '安装Eslint+Prettier+Husky，并且初始化配置。'
            },
            {
                title: 'Typescript', value: 'typescript', description: '初始化Typescript'
            }
        ]
    });
    return response.mod;
